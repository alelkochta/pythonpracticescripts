...
total = 0.0
while again == 'y':
    item = float(input("Enter the expense: "))
    total += item
    again = input("Enter 'y' to add another expense or hit any other key to end: ")
thing = (budget - total)
if thing > 0:
    print("You are", format(thing, '.2f'), "dollars under budget.")
elif thing < 0:
    print("You are", format((thing * -1), '.2f'), "dollars over budget")
else:
